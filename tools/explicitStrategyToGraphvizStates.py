...
def get_true_prop(propositions, values):
  out = []
  for i in range(len(propositions)):
    if values[i]=="1":
      out.append(propositions[i])
...
for line in sys.stdin.readlines():
  if line.startswith("State"):
    ...
    states[stateNo] = (rank, get_true_prop(thesePropositions, theseValues))
...
for (state,(rank,values)) in states.items():
  print("\""+state+"\" [shape=box,label=\""+values+"/"+rank+"\"];")
for state in states:
